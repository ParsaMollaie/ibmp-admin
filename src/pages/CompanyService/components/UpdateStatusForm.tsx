import { updateCompanyServiceStatus } from '@/services/company-service';
...
const statusOptions: { label: string; value: API.CompanyServiceStatus }[] = [
...
const UpdateStatusForm: React.FC<UpdateStatusFormProps> = ({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await updateCompanyServiceStatus(
        record.id,
        values.status,
      );
      ...
      if (response.success) {
        form.resetFields();
        message.success('وضعیت سرویس با موفقیت تغییر کرد');
        onSuccess();
      } else {
