...
const CreateForm: React.FC<CreateFormProps> = ({
  ...
  const [users, setUsers] = useState<API.OrderUser[]>([]);
  const [loadingUsers, setLoadingUsers] = useState(false);
  const [userSearchTimeout, setUserSearchTimeout] = useState<ReturnType<
    typeof setTimeout
  > | null>(null);
  ...
  const handleSubmit = async () => {
    try {
      ...
      const response = await createContactProfile(payload);

      if (response.success) {
        message.success('پروفایل تماس با موفقیت ایجاد شد');
        form.resetFields();
        setCitiesMap({});
        setUsers([]);
        onSuccess();
      } else {
        message.error(response.message || 'خطا در ایجاد پروفایل تماس');
      }
    } catch (error) {
            ...
            <Col span={12}>
              ...
              >
                <Select
                  ...
                  notFoundContent={
                    loadingUsers ? 'در حال جستجو...' : 'کاربری یافت نشد'
                  }
                  options={users.map((u) => ({
