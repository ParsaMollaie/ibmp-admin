...
import { updateServiceCategory } from '@/services/service';
...
const UpdateCategoryForm: React.FC<UpdateCategoryFormProps> = ({
  ...
  // Pre-populate from record's category chain
  useEffect(() => {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      const response = await updateServiceCategory(record.id, categoryId);
      if (response.success) {
